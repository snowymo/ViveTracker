...
module.exports = (
  ...
  unicastAddresses = ['192.168.1.106', '192.168.1.105', '192.168.1.104', '192.168.1.103', '192.168.1.102', '192.168.1.100']
) => { return new Holojam(
  mode, serverAddress, webPort, upstreamPort, downstreamPort, multicastAddress, unicastAddresses
);};
...
var Holojam = function(
  ...
  const Emit = (nugget, buffer) => {
	...
	if (multicast) {
		...
		unicastAddresses.forEach((ucAddress) => {
			udp.send(buffer, 0, buffer.length,
			  sendPort, ucAddress,
			  (error, bytes) => { if(error) throw error; }
			);
